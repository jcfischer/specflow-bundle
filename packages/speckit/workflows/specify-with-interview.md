...
When `/speckit.specify <feature>` is invoked:
...
Do you want to:
1. Approve and proceed to /speckit.plan
...
```
User: /speckit.specify add a delete button to user profiles
...
- **SpecKit Templates**: Uses `templates/spec.md` as base structure
- **Spec Registry**: Records interview completion in `spec-registry.json`

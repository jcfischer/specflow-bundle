...
export class SQLiteAdapter implements DatabaseAdapter {
  ...
  private initializeSchema(): void {
    ...
    // Migration: add migrated_from column if it doesn't exist
    try {
  ...
  async listFeatures(filters?: FeatureFilters): Promise<Feature[]> {
    ...
    const rows = db.query<FeatureRow, any[]>(query).all(...params);
  ...
  async updateFeatureStatus(id: string, status: FeatureStatus): Promise<void> {
    ...
    if (startedAt && completedAt) {
      ...
      db.run(`UPDATE features SET status = ? WHERE id = ?`, [status, id]);
    }
  }
  ...
  async getHardenResults(featureId: string): Promise<HardenResult[]> {
    ...
    const rows = db.query<
      ...
      [string]
    >(`SELECT * FROM harden_results WHERE feature_id = ? ORDER BY id ASC`).all(
      featureId
    );
  ...
  async getPendingApprovals(): Promise<ApprovalGate[]> {
    ...
    const rows = db.query<
      ...
      []
    >(`SELECT * FROM approval_gates WHERE status = 'pending' ORDER BY triggered_at ASC`).all();
  ...
  async insertApprovalGate(featureId: string): Promise<void> {
    const db = this.getDb();
    db.run(`INSERT INTO approval_gates (feature_id, status) VALUES (?, 'pending')`, [
      featureId,
    ]);
  }
  ...
  // ============================================
  // Version Control Operations (No-ops)
  // ============================================
  ...
  private rowToFeature(row: FeatureRow): Feature {
